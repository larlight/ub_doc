LArLight is a very simple \CPP extention package based on \ROOT. 
...
\begin{itemize}
    \item Operating Systems \& Compilers ... to use {\ttfamily C++11} features
      \begin{itemize}
          \item Mac OSX 10.6 (Darwin 10.8) or later with \clang 3.1 or later
          \item Linux 3.11 or later with \gpp 4.3 or later
      \end{itemize}

    \item \python 2.6 or later (but not 3.X) with headers/libraries
      \begin{itemize}
...
If you are to obtain/update those, you might want to do them in order as well. For instance, install \clang, install \python 2.7, then compile \ROOT from source (to get \PyROOT compilation with your new \python), and then compile LArLight.
